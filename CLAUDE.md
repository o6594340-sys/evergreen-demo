...
- **Реальная рабочая ссылка, отправленная клиенту** (Firestore-синхронизация, доступна с любых устройств):
  ...
  - Вход в админку (общий на контакт-персону и её директора): **только пароль** `evergreen2026` — email больше не спрашивается на экране, внутри зашит фиксированный `evergreen.life.demo@mice-app.ru` (константа `FIXED_ADMIN_EMAIL` в admin.js), Firebase-аутентификация продолжает работать по нему в фоне
...
- **Кэш-бастинг:** все локальные `<script src="js/...">` и `<link href="css/...">` в `index.html`/`admin.html`/`dashboard.html` теперь имеют `?v=N`. **При каждом изменении JS/CSS — поднимать номер версии во ВСЕХ файлах, где встречается этот скрипт/стиль**, иначе браузер/CDN отдают смешение старого и нового кода (несколько раз ловили баги именно из-за этого — например, HTML без поля email + закэшированный JS, который его ещё ждал, вызывал молчаливый краш при входе).
